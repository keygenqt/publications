...
Но Docker нам понадобится не только для сборки - он также нужен будет для установки [Aurora SDK](https://developer.auroraos.ru/downloads/sdk_bt/5.1.6.110/linux). На данный момент поддержка macOS с процессором Apple silicon (aarch64) есть только в бета-версии Aurora SDK, последняя доступная сейчас 5.2.75, так что мы будем работать с бетой. Возможно на момент чтения 5.2 будет уже стабильной и вам будет проще, сейчас версия дорабатывается и возможны мелкие баги. Но если вы опытный разработчик, проблем возникнуть не должно.
...
После успешной сборки в директории `target/x86_64-unknown-linux-gnu/debug` вы найдете собранное приложение. Но у нас три архитектуры (`armv7hl`, `aarch64`, `x86_64`).  Для архитектур `armv7hl`, `aarch64` необходимо указать `linker`. Сделать это можно в `config.toml`, который находится в директории `.cargo` (если его нет - его нужно создать).
