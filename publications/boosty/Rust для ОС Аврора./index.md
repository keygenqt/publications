...
1. Помимо самого `Qt5Network` необходимо подключить и `Qt5Core`.
2. Генерация доступна только для Linux - но работе на других ОС это не мешает, для них ее можно просто отключить.
3. Библиотеки нужно привязать к приложению после выполнения генерации, иначе вы может столкнуться с проблемой `undefined reference`.
